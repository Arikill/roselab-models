...
parameters.short_pass.syne.integration_tau = -0.01;
parameters.short_pass.syni.tau = 0.009;
parameters.short_pass.syni.integration_tau = -0.02;
...
for i = 1:nconditions
    if pulse_rates(i) == 50
        ...
        lin = Neuron2(s.fs, parameters.lin);
        parameters.short_pass.syni.integration_tau = -0.025;
        spd = Neuron2(s.fs, parameters.short_pass);
        % parameters.icn.syne.integration_tau = -0.08;
        % icn = Neuron2(s.fs, parameters.icn);
    end
